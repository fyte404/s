...
export default class ytdl implements CommandablePlugin, MessageablePlugin {
    ...
    readonly MSG = {
        URL: `_Reply to this message and send youtube video URL to download video_`,
        RESOLUTION: `_Please choose which number by replying to this message_`
    } as const
    ...
    async onMessage ({ m }: PluginMsgParam) {
        ...
        if (parseInt(state) == WaitingState.URL) {
            ...
            if (isNaN(maxChoice) || !url) {
                return m.reply(`_
_Error in finding message metadata required for download._
`.trim())
            }
